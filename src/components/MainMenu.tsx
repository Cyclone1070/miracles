...
export function MainMenu({ ...props }: Props) {
	...
	return (
		<div className="w-screen h-dvh relative overflow-hidden flex justify-center items-center p-2">
			...
			{/* Note: Corrected Tailwind syntax for CSS variables, e.g., border-[var(--accent)] */}
			<motion.div
				initial={{ opacity: 0, y: 10, visibility: "hidden" }}
				animate={{ opacity: 1, y: 0, visibility: "visible" }}
				transition={{ delay: 2, duration: 0.8, ease: "easeInOut" }}
				className={`border-2 border-[var(--accent)] bg-black/80 p-8 rounded-xl text-[var(--text)] flex flex-col items-center justify-center gap-4 text-center h-full w-full
				max-w-90 max-h-120 min-w-75
				md:max-w-160 md:max-h-130`}
			>
				<h1 className={`font-bold text-6xl md:text-8xl`}>Miracles</h1>
				<HighlightButton
					className={`rounded-xl m-auto bg-[var(--accent)]
					text-2xl p-3 px-5
				...
				<div className={`relative flex items-center`}>
					<input
						className="border border-[var(--accent)] rounded-sm p-1 px-2 outline-0"
						placeholder="Enter your API key"
						type="text"
					/>
					<HighlightButton
						className={`absolute -right-10 w-7 h-7 flex items-center justify-center rounded-full bg-[var(--accent)]`}
						onClick={() => {
				...
				<HighlightButton className={`bg-[var(--accent)] p-1.5`}>
					Clear Data
				</HighlightButton>
			...
			<AnimatePresence>
				{isHelpVisible && (
					...
					>
						<div
							className={`bg-[var(--theme-bg)] w-70 p-4 border-2 border-[var(--accent)] rounded-md text-[var(--text)]`}
						>
